...
chrome.runtime.onInstalled.addListener(({ reason: e, previousVersion: t }) => {
  "update" === e &&
    (t = +t.replace(/\./g, "")) < 31 &&
    chrome.storage.sync.get("uuid", async (e) => {
      if (!e.uuid) {
      ...
      return (
        (function (e) {
          e.onMessage.addListener(
            async ({ msgId: t, service: o = "netflix", data: a } = {}) => {
              const d = a.id;
              let u = !0;
              if (d) {
                const { imdb: e, douban: t } = await (async function (
                  e,
                  t = "netflix"
                ) {
                  let o, a;
                  const s = await ((d = n[t] + e),
                  new Promise((e, t) => {
                    chrome.storage.local.get(d, (n) => {
                      Object.keys(n).length
                        ? e("object" == typeof d ? n : n[d])
                        : e(),
                  ...
                  if (s) (o = s.imdb), (a = s.douban);
                    ...
                    try {
                      const n = await (function ({ id: e, service: t }) {
                        return c(`${i}/${t}/${e}`, r);
                      })({
                        id: e,
                        service: t,
              ...
              (movie = await (async function ({
                ...
                if (i) a.imdbId = i;
                  ...
                  const e = await (async function ({ id: e }) {
                    const t = await (function (e) {
                      return c(
                        "https://unogs.com/api/title/detail?netflixid=" + e
                      );
                    })(e);
                    if (t && t[0] && t[0].title) {
                      const e = t[0],
                        { year: n, top250: o, top250tv: i } = e,
                        r = p[e.vtype],
                        a = e.title.replace(/&#(\d+);/g, function (e, t) {
                          return String.fromCharCode(t);
                        }),
                        c =
                          e.imdbid && "notfound" !== e.imdbid
                            ? {
                                id: e.imdbid,
                                rating: e.imdbrating,
                ...
                a.rt ||
                  (a.rt = await (async function ({
                    title: e,
                    year: t,
                    type: n,
                  }) {
                    const o =
                        "https://www.rottentomatoes.com/napi/search/?query=" +
                    ...
                    if (r) {
                      ...
                      if (1 !== e.length && t) {
                        const o =
                          e.find(
                            (e) =>
                              e.endYear === t || e.year === t || e.meterScore
                          ) || e[0];
                        return {
                          rating: o.meterScore,
                          url: n + o.url,
  ...
  chrome.runtime.onMessage.addListener(({ type: e } = {}, t, n) => {
    "activation" === e &&
      (!(function (e = !1) {
        const t = +new Date();
        chrome.storage.local.get(
          null,
          ({ date: n, settings: o, hasReviewed: i, ...r }) => {
            if (("number" == typeof n && t - n > 864e5) || e) {
              ...
              const e = Object.keys(r).reduce(
                (e, t) => (/^nf\d+$/.test(t) && (e[t] = r[t]), e),
                {}
              );
...
const o = "__production__",
  i =
    "__production__" === o
      ? "https://us-central1-popcorn-plugin.cloudfunctions.net/api/v1"
      : "http://localhost:5001/popcorn-plugin/us-central1/api/v1",
  r = {
    mode: "cors",
    headers: {
      "X-Popcorn": "true",
...
function s({ doubanId: e, title: t, imdbId: n }) {
  const o = "https://movie.douban.com/j",
    i = o + "/subject_suggest?q=";
  if (e)
    return c("https://movie.douban.com/j/subject_abstract?subject_id=" + e);
...
chrome.runtime.onInstalled.addListener(({ previousVersion: e, reason: t }) => {
  ("update" !== t && "install" !== t) ||
    ((e = e || ""),
    u.forEach((t) => {
      +t.version.replace(/\./g, "") > +e.replace(/\./g, "") && d.push(t);
    }));
}),
...
async function f({ imdbId: e, title: t, year: n, type: o }, i = !0) {
  const r = await (function (
    { imdbId: e, title: t, year: n, type: o },
    i = !0
  ) {
    let r = "http://private.omdbapi.com/?apikey=1e0ce2f6";
    return e
      ? ((r += "&i=" + e), c(r))
      : ((r += (i ? "&s=" : "&t=") + encodeURIComponent(t)),
        o && (r += "&type=" + ("tv" === o ? "series" : "movie")),
        n && (r += "&y=" + n),
        c(r));
  })(
    {
  ...
  if (r.Search && r.Search.length) {
    let e,
      n = r.Search;
    return (
      o && (n = r.Search.filter((e) => o === p[e.Type])),
      n.length && 1 !== n.length && (e = n.find((e) => e.Title === t)),
      (e = e || n[0] || r.Search[0]),
      f({
        imdbId: e.imdbID,
      })
  ...
  r.Ratings.length &&
    r.Ratings.forEach((e) => {
      /tomatoes/i.test(e.Source) && (a = e.Value),
        /metacritic/i.test(e.Source) && (s = e.Value);
    });
  const u = a
    ? {
